...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_shape, outputs_count, kernels_count = [32, 32, 64, 64], residual_blocks = [0, 0, 0, 0]):
        ...
        ratio           = 2**4
        fc_inputs_count = kernels_count[3]*((input_width)//ratio)*((input_height)//ratio)

        self.layers_features = []
        self.layers_features.append(nn.Conv2d(input_channels, kernels_count[0], kernel_size=3, stride=1, padding=1))
        self.layers_features.append(nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
        self.layers_features.append(nn.ReLU())
        for i in range(residual_blocks[0]):
            self.layers_features.append(ResidualBlock(kernels_count[0]))
            
        self.layers_features.append(nn.Conv2d(kernels_count[0], kernels_count[1], kernel_size=3, stride=1, padding=1))
        self.layers_features.append(nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
        self.layers_features.append(nn.ReLU())
        for i in range(residual_blocks[1]):
            self.layers_features.append(ResidualBlock(kernels_count[1]))
            
        self.layers_features.append(nn.Conv2d(kernels_count[1], kernels_count[2], kernel_size=3, stride=1, padding=1))
        self.layers_features.append(nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
        self.layers_features.append(nn.ReLU())
        for i in range(residual_blocks[2]):
            self.layers_features.append(ResidualBlock(kernels_count[2]))

        self.layers_features.append(nn.Conv2d(kernels_count[2], kernels_count[3], kernel_size=3, stride=1, padding=1))
        self.layers_features.append(nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
        self.layers_features.append(nn.ReLU())
        for i in range(residual_blocks[3]):
            self.layers_features.append(ResidualBlock(kernels_count[3]))

        self.layers_features.append(nn.Flatten()) 
