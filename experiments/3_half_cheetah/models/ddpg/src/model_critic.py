...
class Model(torch.nn.Module):
    def __init__(self, input_shape, outputs_count, hidden_count = 256):
        ...
        self.layers = [ 
                        nn.Linear(outputs_count + hidden_count, hidden_count//2),
                        nn.ReLU(),            
    ...
    def save(self, path):
        ...
        torch.save(self.model_features.state_dict(), path + "trained/model_features_critic.pt")
        torch.save(self.model.state_dict(), path + "trained/model_critic.pt")

    def load(self, path):       
        print("loading from ", path)
        self.model_features.load_state_dict(torch.load(path + "trained/model_features_critic.pt", map_location = self.device))
