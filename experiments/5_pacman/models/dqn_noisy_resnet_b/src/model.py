...
class Model(torch.nn.Module):
    ...
    def forward(self, state):
        self.features    = self.model_features(state)

        value       = self.model_value(self.features)
        advantage   = self.model_advantage(self.features)
    ...
    def get_activity_map(self, state):
        features    = self.features.reshape((1, 128, 6, 6))
